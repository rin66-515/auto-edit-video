...
class WorkerRecoveryTest(unittest.TestCase):
    ...
    def test_app_restart_does_not_recover_live_visual_stage(self):
        db.init_db();project_id=self.create_project("revision_planning","revision_requested","修改剪辑方案")
    ...
    def test_app_restart_recovers_its_own_render_stage(self):
        db.init_db();project_id=self.create_project("rendering","render_requested","成片渲染")
        db.recover_interrupted_projects("app")
