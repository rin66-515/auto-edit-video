...
function controlsHtml(p){
  ...
  if(renderStage||p.status==='draft_ready'){
    const longLabel=longCount?`继续渲染 ${esc(longVersion||'')} · 长篇`:'单独渲染长篇';
    const shortLabel=shortCount?`继续渲染 ${esc(shortVersion||'')} · 短篇`:'单独渲染短篇';
    const renderButtons=`<button class="control-render-long" data-generate="long_16x9" ${renderState?'':'disabled'}>${longLabel}</button><button class="control-render-short" data-generate="short_9x16" ${renderState?'':'disabled'}>${shortLabel}</button><button class="control-render-both" data-generate="both" ${renderState?'':'disabled'}>长短篇一起渲染</button>`;
...
async function show(id){
  ...
  $('#detail').innerHTML=`
    ...
    <div class="card"><h3>剪辑方案重规划意见</h3><p class="muted">先选择关联版本再填写意见。点击该版本的“废弃并重规划”时，只会读取绑定到它的待应用意见；生成后会记录“来源版本 → 新版本”。未关联版本的意见用于项目级重规划。</p><div class="row replan-controls"><select id="revision-target"><option value="">项目级（不绑定版本）</option>${replanTargets.map(e=>`<option value="${e.id}">基于 ${esc(e.version)} · ${esc(formatNames[e.format]||e.format)}</option>`).join('')}</select><select id="kind"><option value="edit">剪辑结构</option><option value="shot">镜头取舍</option><option value="duration">时长/章节</option><option value="style">风格/节奏</option></select><button id="open-replan-dialog">填写重规划意见</button></div><ul>${p.revisions.map(r=>`<li class="revision-item"><span><b>${esc(r.kind)}</b> ${esc(r.body)} · ${esc(revisionStatusLabel(r))}</span><button class="danger small" data-delete-revision="${r.id}" data-revision-status="${esc(r.status)}" ${canManageRevisions?'':'disabled'} title="${canManageRevisions?'删除该条规划意见；已完成的意见不会回滚现有方案或版本':'请先暂停或停止项目'}">删除意见</button></li>`).join('')||'<li>暂无重规划意见</li>'}</ul></div>
  ...
  $('#open-replan-dialog').onclick=()=>{const target=$('#revision-target').selectedOptions[0]?.textContent||'项目级（不绑定版本）',kind=$('#kind').selectedOptions[0]?.textContent||'剪辑结构';$('#replan-dialog-meta').textContent=`关联：${target}　·　类型：${kind}`;replanDialog.showModal();setTimeout(()=>$('#revision').focus(),0)};
  $('#send').onclick=async()=>{if(!$('#revision').value.trim()){ $('#revision').focus();return }try{const sourceExportId=Number($('#revision-target').value)||null;const result=await api(`/api/projects/${p.id}/revisions`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({kind:$('#kind').value,body:$('#revision').value.trim(),source_export_id:sourceExportId})});replanDialog.close();alert(result.source_version?`意见已绑定到 ${result.source_version}。随后点击该版本的“废弃并重规划”即可应用。`:'项目级意见已记录；点击启动项目后将重新规划。');await show(p.id)}catch(e){alert(e.message)}};
